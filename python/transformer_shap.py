"""
SHAP-based interpretability utilities for the MassLinker Transformer classifier.

This module provides visualization and analysis functions for interpreting
Transformer-based MassLinker classifiers with SHAP values. MassLinker represents
each sample as a sequence of compound-level metabolic tokens, where each token
contains RBF-derived features describing chromatographic peak height, position,
and width.

The functions in this file summarize SHAP values at different levels:
sample-level, token-level, parameter-group-level, and class-wise importance.
They support bar plots, stacked importance plots, two-dimensional SHAP heatmaps,
and DeepExplainer-based SHAP value calculation for Transformer models.
"""
...
def plot_time_step_bar_chart(shap_values, n_test, top_n_time_steps, feature_labels=None, stacked=True, colors=None,
                             save_path=None):
    """
    Plot class-wise SHAP importance for the top-ranked sequence positions.

    This function aggregates absolute SHAP values across token-level features,
    ranks MassLinker sequence positions by total importance, and visualizes the
    top positions as either a stacked or grouped bar chart.

    In the MassLinker Transformer model, each sequence position corresponds to
    a compound-associated metabolic token. The feature dimension contains
    RBF-derived parameters for chromatographic peak height, position, and width.

    Parameters
    ----------
    shap_values : np.ndarray
        SHAP value array with shape total_rows x n_features x n_classes.
    n_test : int
        Number of test samples used to calculate SHAP values.
    top_n_time_steps : int
        Number of top sequence positions to visualize.
    feature_labels : list, optional
        Labels for sequence positions. If None, sequence indices are used.
    stacked : bool, optional
        Whether to draw a stacked bar chart. If False, grouped bars are used.
    colors : list, optional
        Class-specific colors.
    save_path : str, optional
        Output PDF path. If None, the figure is shown interactively.

    Returns
    -------
    np.ndarray
        Indices of sequence positions ordered by total SHAP importance.
    """
    total_rows, n_features, n_classes = shap_values.shape
    seq_len = total_rows // n_test

    # Restore sample-by-sequence structure from flattened SHAP rows.
    shap_reshaped = shap_values.reshape(n_test, seq_len, n_features, n_classes)

    # Aggregate absolute SHAP values across the feature dimension.
    importance_by_class = np.sum(np.abs(shap_reshaped), axis=2)
    mean_importance_by_class = np.mean(importance_by_class, axis=0)

    # Rank sequence positions by total class-wise importance.
...
def plot_aggregated_time_importance(shap_values, n_test, top_n_time_steps, feature_labels=None, colors=None,
                                    save_path=None):
    """
    Plot aggregated class-wise SHAP importance for top sequence positions.

    This function ranks sequence positions by total SHAP importance and draws
    a stacked area plot showing how different output classes contribute to the
    importance of each selected MassLinker token.
    ...
    # Restore sample-by-sequence SHAP structure.
    shap_reshaped = shap_values.reshape(n_test, seq_len, n_features, n_classes)

    # Aggregate absolute SHAP values across RBF-derived features.
    importance_by_class = np.sum(np.abs(shap_reshaped), axis=2)
    mean_importance_by_class = np.mean(importance_by_class, axis=0)

    # Rank sequence positions by total importance across classes.
    total_importance = np.sum(mean_importance_by_class, axis=1)
    order_index = np.argsort(total_importance)
    top_n_indices = order_index[-top_n_time_steps:].tolist()
...
def plot_top_n_feature_shap_2d(shap_values, n_test, top_n_time_steps, target_class_index, feature_labels=None,
    ...
    """
    Plot two-dimensional SHAP heatmaps for top-ranked MassLinker tokens.

    This function visualizes SHAP values for the most important sequence
    positions. Each selected MassLinker token is decomposed into three RBF
    parameter groups: peak height, peak position, and peak width. Each group
    contains 20 RBF components.

    For each test sample, the top tokens are selected according to the sum of
    absolute SHAP values across the 60 RBF-derived features. An additional row
    shows the average SHAP pattern across all test samples.

    Parameters
    ----------
    shap_values : np.ndarray
        SHAP values from either machine-learning or Transformer-based analysis.
    n_test : int
        Number of test samples.
    top_n_time_steps : int
        Number of top MassLinker tokens to visualize.
    target_class_index : int
        Output class index to visualize.
    feature_labels : list, optional
        Labels for MassLinker tokens.
    save_path : str, optional
        Output PDF path. If None, the figure is shown interactively.
    mode : str, optional
        SHAP value organization mode. `ML` expects flattened ML-style SHAP
        values, while other values use Transformer-style token SHAP values.
    ...
    for test_time in range(n_test):
        ...
        # Select top tokens for each individual test sample.
    ...
    # Calculate average SHAP patterns across all test samples.
    ...
    for row_idx in range(num_rows):
        ...
        for col_idx in range(NUM_MAIN_FEATURES):
            ...
            # Use a symmetric color scale centered at zero for signed SHAP values.
...
def run_shap_analysis(model_path, data_path, device, n_background=50, n_test=3):
    """
    Run DeepExplainer-based SHAP analysis for a trained MassLinker Transformer model.

    This function loads a serialized MassLinker dataset, reshapes the token
    tensor into Transformer input format, samples background and test examples,
    loads a trained Transformer model, and calculates SHAP values for selected
    test samples.

    The input tensor is reshaped into:
    sample_count x 5194 tokens x 60 RBF-derived features.

    Parameters
    ----------
    model_path : str
        Path to the saved Transformer model state dictionary.
    data_path : str
        Path to a joblib-serialized MassLinker dataset.
    device : torch.device
        Device used for model inference and SHAP calculation.
    n_background : int, optional
        Number of background samples used by SHAP DeepExplainer.
    n_test : int, optional
        Number of test samples selected for SHAP interpretation.

    Returns
    -------
    tuple
        final_shap_values : list
            Class-wise SHAP value arrays.
        test_samples : np.ndarray
            Test samples used for SHAP analysis.
        num_classes : int
            Number of output classes inferred from the dataset.
    """
    dataset = joblib.load(data_path)

    x_raw = dataset.samples

    # Reshape MassLinker tokens into Transformer input format.
    x_tensor = torch.tensor(x_raw, dtype=torch.float32).reshape(x_raw.shape[0], 5194, 60)

    total_idx = np.arange(len(x_tensor))

    # Randomly select background and test samples for DeepExplainer.
    ...
    # Infer the number of output classes from the dataset.
